...
export default function AdminKyc() {
  ...
  return (
    <>
      <PageHeader kicker="Operations Queue" title="KYC Verifications" description="Approve, reject, or escalate document submissions across the network."
        actions={
          <div className="flex gap-2">
            <button onClick={() => toast.success("Bulk approved 12 documents")} className="px-3.5 py-2 rounded-xl glass text-sm hover:bg-success/15 hover:text-success border border-success/20"><Check className="h-4 w-4 inline mr-1.5" />Approve all</button>
            <button onClick={() => toast.error("Bulk rejected 3 documents")} className="px-3.5 py-2 rounded-xl glass text-sm hover:bg-danger/15 hover:text-danger border border-danger/20"><X className="h-4 w-4 inline mr-1.5" />Reject all</button>
          </div>
        } />
                  ...
                  <tr className="bg-slate-900/[0.02]">
                    <td colSpan="5" className="px-5 py-5">
                      <div className="grid md:grid-cols-2 gap-5">
                        <div className="glass rounded-xl p-4 h-56 grid place-items-center">
                          <div className="text-center">
                            <FileText className="h-12 w-12 text-cyan-glow mx-auto mb-2 opacity-60" />
                            <div className="text-sm">{d.name}</div>
                            <div className="text-xs text-muted-foreground mt-1">Document preview</div>
                          </div>
                        </div>
                        <div>
                          <div className="text-xs font-mono uppercase tracking-widest text-cyan-glow mb-2">OCR extraction</div>
...
function RejectModal({ onClose }) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="glass-strong rounded-3xl w-full max-w-md p-6 shadow-card">
