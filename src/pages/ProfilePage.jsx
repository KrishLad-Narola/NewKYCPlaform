    ...
    return (
      <div className="min-h-screen grid place-items-center">
        <div className="text-center">
          ...
          <Link to="/" className="text-cyan-glow">Go home</Link>
        </div>
      </div>
    );
  }
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 lg:px-12 py-12">
        ...
        <div className="glass-strong rounded-3xl p-8 shadow-card">
          <div className="flex flex-wrap items-start gap-8">
            <div className="h-20 w-20 rounded-2xl bg-gradient-to-br from-primary to-cyan-glow grid place-items-center text-2xl font-display font-semibold shadow-glow">{b.logo}</div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-primary to-cyan-glow text-white text-sm font-medium shadow-glow">
            ...
            <div className="glass rounded-2xl p-5">
              ...
              {riskFlags.slice(0, 2).map((f) => (
                <div key={f.id} className="text-xs text-muted-foreground py-1.5 flex items-center gap-2">
                  <span className={`h-1.5 w-1.5 rounded-full ${f.severity === "high" ? "bg-danger" : f.severity === "medium" ? "bg-warning" : "bg-cyan-glow"}`} />
                  {f.title}
                </div>
              ))}
              <button className="mt-2 text-xs text-cyan-glow inline-flex items-center gap-1 hover:gap-2 transition-all">View full report <ArrowRight className="h-3 w-3" /></button>
            </div>
          </div>
        </div>
      </main>
